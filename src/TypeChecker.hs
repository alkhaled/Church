...
check (Pair e1 e2)  context = do t1 <- check e1 context
                                 t2 <- check e2 context
                                 return (TPair t1 t2)
                                 
check (Fst e)       context = do x <- freshVar
                                 y <- freshVar
                                 t <- check e context
                                 tell [Equal t (TPair (TVar x) (TVar y))]
                                 return (TVar x)

check (Snd e)       context = do x <- freshVar
                                 y <- freshVar
                                 t <- check e context
                                 tell [Equal t (TPair (TVar x) (TVar y))]
                                 return (TVar y)

check (App e1 e2)   context = do tvar <- freshVar
                                 t1 <- check e1 context
                                 t2 <- check e2 context
                                 tell [Equal t1 (TArrow t2 (TVar tvar))]
                                 return (TVar tvar)

check (Lambda x e)  context = do t1 <- freshVar 
                                 t2 <- check e (Map.insert x (newScheme t1) context)
                                 return(TArrow (TVar t1) t2)  

check (Let f e1 e2) context = do (t1, context') <- quantify (check e1 context) context  
                                 t2 <- check e2 (Map.insert f t1 context')
                                 return t2
                               
check (Output e)    context = do t <- check e context
                                 return TUnit
                               
check (Concat e1 e2) context = do t1 <- check e1 context
                                  t2 <- check e2 context
...
check (Throw e) context = do x <- freshVar
                             t <- check e context
                             tell[Equal t TString]
                             return (TVar x)
                                
check (Try e exc onFail) context = do te <- check e context
                                      tFail <- check onFail context 
                                      tell[Equal te tFail]
                                      return te
-- TODO : CLean this up                                                    
check (LetRec f (Lambda x e1) e2)     context = do tvar_x <- freshVar
                                                   tvar_resf <- freshVar
                                                   let fType = (emptyScheme (TArrow (TVar tvar_x) (TVar tvar_resf)))
                                                   t1 <- check e1 (Map.insert f fType
                                                                  (Map.insert x (newScheme tvar_x) 
                                                                  context))  
                                                   t2 <- check e2 (Map.insert f fType context)
                                                   tell [Equal t1 (TVar tvar_resf)]
                                                   return t2                                  

check (If cond thenBranch elseBranch) context = do tCond <- check cond context
...
arithCheck n1 n2 context = do t1 <- check n1 context
                              t2 <- check n1 context
                              tell [Equal t1 TInt]
                              tell [Equal t2 TInt]
                              return TInt
...
bExpCheck n1 n2 context = do t1 <- check n1 context
                             t2 <- check n1 context
                             tell [Equal t1 TInt]
                             tell [Equal t2 TInt]
                             return TBool
